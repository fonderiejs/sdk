	...
	return {
		...
		updateProfile: async (ctx: IFonderieContext): Promise<Response> => {
			...
			if (Object.keys(fields).length === 0) {
				return setApiResponse(HTTP.UNPROCESSABLE, 'INVALID_PARAMETER', 'Provide at least one of: firstName, lastName, avatarUrl');
			}
		...
		updatePreferences: async (ctx: IFonderieContext): Promise<Response> => {
			...
			if (!row) {
				return setApiResponse(HTTP.UNPROCESSABLE, 'INVALID_PARAMETER', 'Provide at least one preference field');
			}
		...
		updateEmail: async (ctx: IFonderieContext): Promise<Response> => {
			...
			if (typeof newEmail !== 'string' || !newEmail.includes('@')) {
				return setApiResponse(HTTP.UNPROCESSABLE, 'INVALID_PARAMETER', 'A valid email address is required');
			}
			...
			if (normalised === oldEmail) {
				return setApiResponse(HTTP.UNPROCESSABLE, 'INVALID_PARAMETER', 'New email must differ from current email');
			}
