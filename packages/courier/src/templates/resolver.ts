...
export class FSTemplateResolver implements ITemplateResolver {
	...
	async resolve(
		...
		const [text, html, subject] = await Promise.all([
			localePrefix
				? readOptional(join(this.directory, `${localePrefix}.txt`))
					.then(v => v ?? readOptional(join(this.directory, `${type}.txt`)))
				: readOptional(join(this.directory, `${type}.txt`)),

			localePrefix
				? readOptional(join(this.directory, `${localePrefix}.html`))
					.then(v => v ?? readOptional(join(this.directory, `${type}.html`)))
				: readOptional(join(this.directory, `${type}.html`)),

			localePrefix
				? readOptional(join(this.directory, `${localePrefix}.subject.txt`))
					.then(v => v ?? readOptional(join(this.directory, `${type}.subject.txt`)))
